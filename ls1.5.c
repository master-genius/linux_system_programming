...
int out_control(struct format_info fi);
...
void help(void)
{
    char *help_info[] = {
        "显示文件/目录信息,类似于ls命令。\n",
        "参数/选项：-l -a -r -s -m -f -i -p -t --deep --lnk\n",
    ...
    while (cur!=NULL) {
        ...
        if (cur->is_dir==0) {
            if (lstat(cur->path, &sttmp) < 0){
                ...
                memcpy(&one_buf.st, &sttmp, sizeof(sttmp));
        ...
        if (_args[ARGS_PATH]==0 && _args[ARGS_SHOWSTATS]==0)
            printf("%s/:\n",cur->path);
...
void out_info(struct file_buf * fb, struct format_info fi) {
    ...
    if (_args[ARGS_PATH] && strlen(fb->path)>0)
        printf("%s/",fb->path);
    ...
    if (_args[ARGS_TYPE]) {
        ...
        if (strlen(fb->path)>0) {
            strcpy(name_buf, fb->path);
            strcat(name_buf, "/");
        }
        strcat(name_buf, fb->name);
        if (S_ISDIR(fb->st.st_mode))
            ...
            flag = TYPE_BLK;
        else if (S_ISREG(fb->st.st_mode) && access(name_buf,X_OK)==0)
            flag = TYPE_EXEC;
        if (flag > 0)
            printf("%c",flag);
    }
    ...
    if (S_ISLNK(fb->st.st_mode) 
        ...
        if ((MAX_NAME_LEN - (strlen(fb->path) + 1 + strlen(fb->name))) > 0) {
            strcat(fb->path, "/");
            strcat(fb->path, fb->name);
