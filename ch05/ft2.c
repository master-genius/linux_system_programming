...
int fbuf_name_comp(const void *a, const void *b) {
    int alen = strlen((*(struct file_buf**)a)->name);
    int blen = strlen((*(struct file_buf**)b)->name);
    return memcmp((*(struct file_buf**)a)->name, 
            (*(struct file_buf**)b)->name, 
            ( (alen>blen) ? alen : blen )
        );
