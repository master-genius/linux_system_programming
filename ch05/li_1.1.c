...
void caclt_fi(struct file_buf * fbuf, struct format_info *fi, ino_t ino) {
    ...
    len_buf = (unsigned long)log10((double)ino);
    ...
    while (pl) {
        ...
        for(i=0; i<pl->end_ind; i++) {
            ...
            while((rd=readdir(d))!=NULL) {
                ...
                caclt_fi(&fbuf, &_aic.fi, stmp.st_ino);
                /*
                len_buf = (fbuf.name_is_ch ? fbuf.name_outlen : fbuf.name_len);
                ...
                len_buf = (unsigned long int)log10(stmp.st_ino);
                if (_aic.fi.max_ino_bits < len_buf)
                    _aic.fi.max_ino_bits = len_buf;
                */
